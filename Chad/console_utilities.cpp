...
void ConsoleUtilities::PrintEndpointNames(const std::vector<Endpoint>& render_devices, const std::vector<Endpoint>& recording_devices) {
	...
	std::wcout << str_render_dev << std::setw(longest_name_len + 4) << str_capture_dev << std::endl;
	for (auto i = 0; i < num_capture_dev; i++) {
		if (i < num_render_dev) {
			std::wcout << std::setw(longest_name_len + 3) << std::left << render_devices.at(i).device_name;
		}
		// if there are still capture devices to list, but no more render devices then
		// output an empty string, so recording devices are listed in correct position
		if (i >= num_render_dev&& i < num_capture_dev) {
			std::wcout << std::setw(longest_name_len + 3) << std::left << L" ";
		}
